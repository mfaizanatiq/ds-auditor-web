...
(function (global) {
  ...
  var SKIP_TAGS = {
    SCRIPT: 1, STYLE: 1, NOSCRIPT: 1, SVG: 1, PATH: 1, META: 1, LINK: 1,
    HEAD: 1, HTML: 1, IFRAME: 1, CANVAS: 1, VIDEO: 1, AUDIO: 1,
  };
  ...
  function auditColorProp(el, elementRef, tokens, tokenNames, issues, seen, selector, label, cs, prop, isText, ctx) {
    ...
    issues.push({
      ...
      selector: selector,
      found: val,
      message: E.colorRoleMessage(usageRole),
      fixes: fixes,
    });
  }
        ...
        if (!seen.has(typoSig)) {
          ...
          if (typoFixes.length) {
            ...
            issues.push({
              ...
              message: 'Hardcoded typography — use a type style token (e.g. body-medium).',
              fixes: typoFixes,
            });
          }
        }
