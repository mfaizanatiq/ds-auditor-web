...
(function (global) {
  ...
  function hasDirectVisibleText(el) {
    if (!el) return false;
    // Prefer own text nodes so parent/child both get fair contrast checks.
    for (var i = 0; i < el.childNodes.length; i++) {
      var n = el.childNodes[i];
      if (n.nodeType === 3 && trimText(n.textContent)) return true;
    }
    var tag = el.tagName;
    if (tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'BUTTON' || tag === 'A' || tag === 'LABEL' ||
        tag === 'P' || tag === 'SPAN' || tag === 'LI' || tag === 'TD' || tag === 'TH' ||
        HEADING_TAGS[tag]) {
  ...
  function effectiveBackground(el, cs, cache) {
    if (!cache.bgMap) cache.bgMap = typeof WeakMap !== 'undefined' ? new WeakMap() : null;
    if (cache.bgMap && cache.bgMap.has(el)) return cache.bgMap.get(el);
    ...
    if (cache.bgMap) cache.bgMap.set(el, bg);
    return bg;
  }
  ...
  function documentHasFocusStyles(doc) {
    if (!doc) return false;
    try {
      ...
      for (var i = 0; i < sheets.length; i++) {
        ...
        if (!rules) continue;
        for (var j = 0; j < Math.min(rules.length, 400); j++) {
          var sel = rules[j].selectorText || '';
          if (/:focus(-visible)?\b/.test(sel)) return true;
        }
      }
    } catch (e) { /* ignore */ }
    return false;
  ...
  function auditElement(el, ctx) {
    ...
    if (!hidden && (isNative || role === 'button' || role === 'link' || tabIndex === '0' || tabIndex === 0)) {
      // Only flag when the page has no :focus / :focus-visible styles AND idle outline is cleared.
      // Many design systems hide default outline and restore it under :focus-visible.
      if (ctx.hasFocusStyles === false) {
        var outlineNone = cs.outlineStyle === 'none' || parseFloat(cs.outlineWidth) === 0;
        var noShadow = !cs.boxShadow || cs.boxShadow === 'none';
        var noOutlineOffsetRing = !cs.outlineOffset || cs.outlineOffset === '0px';
        var inlineClearsOutline = /outline\s*:/.test(el.getAttribute('style') || '');
        if (outlineNone && noShadow && (inlineClearsOutline || noOutlineOffsetRing)) {
  ...
  function runA11yAudit(pageMeta) {
    ...
    var ctx = {
      ...
      cache: {},
      hasFocusStyles: documentHasFocusStyles(doc),
    };
